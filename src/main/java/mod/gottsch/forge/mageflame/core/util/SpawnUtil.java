...
import mod.gottsch.forge.mageflame.core.persistence.PlayerData;
import mod.gottsch.forge.mageflame.core.persistence.StateSaverAndLoader;
import mod.gottsch.forge.mageflame.core.persistence.SummonedEntityData;
...
public class SpawnUtil {
    ...
    public static <T extends Mob & ISummonedEntity>Optional<?> spawnAtPos(ServerLevel world, RandomSource random, LivingEntity owner, EntityType<T> entityType, Vec3 coords) {
        return spawnAtPos(world, random, owner, entityType, new Coords((int)coords.x, (int)coords.y, (int)coords.z));
    }
     ...
     */
    public static <T extends Mob & ISummonedEntity>Optional<?> spawnAtPos(ServerLevel world, RandomSource random, LivingEntity owner, EntityType<T> entityType, ICoords coords) {
        ...
        if (!world.isClientSide) {
            ...
            SpawnPlacements.Type placement = SpawnPlacements.getPlacementType(entityType);
            if(NaturalSpawner.isSpawnPositionOk(placement, world, spawnPos, entityType)) {
                // create entity
                Mob mob = entityType.create(world);
                if (mob != null) {
